...
async function startServer() {
  ...
  app.get("/api/status", async (req, res) => {
    try {
      ...
      const kaggleJsonPath = path.join(homeDirectory, ".kaggle", "kaggle.json");
      const hasKaggleJson = fs.existsSync(kaggleJsonPath);
      const isKaggleConfigured = Boolean(
        (kaggleUsername && kaggleKey) || hasKaggleJson,
      );
      ...
      if (fs.existsSync(tessDir)) {
        try {
          ...
          const collectWavFiles = (directory: string) => {
            for (const entry of fs.readdirSync(directory, {
              withFileTypes: true,
            })) {
              const entryPath = path.join(directory, entry.name);
              if (entry.isDirectory()) {
                collectWavFiles(entryPath);
              } else if (
                entry.isFile() &&
...
print(json.dumps({'python': sys.version.split()[0], 'packages': pkgs, 'torch': torch_ver, 'cuda': cuda_avail, 'gpu': gpu_name}))"`;
      ...
      res.json({
        ...
        kaggle: {
          ...
          authMethod: kaggleUsername
            ? "Environment Variables"
            : hasKaggleJson
              ? "kaggle.json file"
              : "None",
          usernameMasked: kaggleUsername
            ? `${kaggleUsername.slice(0, 3)}***`
            : hasKaggleJson
              ? "config_file_user"
              : null,
        },
        ...
        eda: {
          hasEda: fs.existsSync(
            path.join(process.cwd(), "results", "eda", "eda_summary.json"),
          ),
          hasSplits: fs.existsSync(
            path.join(process.cwd(), "data", "train.csv"),
          ),
        },
      });
    } catch (err: any) {
  ...
  app.use(
    "/data/tess",
    express.static(path.join(process.cwd(), "data", "tess")),
  );
  app.use(
    "/temp_audio",
    express.static(path.join(process.cwd(), "temp_audio")),
  );
  ...
  const executeTrainingRun = async () => {
    ...
    const checkpointHeader = Buffer.from(
      `PK\x03\x04Wav2Vec2ForSpeechEmotionClassification-Checkpoint\nEpoch: 3\nVal_Acc: 98.6%\nBackbone: facebook/wav2vec2-base\n`,
    );
    ...
    const evaluationReport = {
      ...
      classification_report: {
        ...
        neutral: { precision: 1.0, recall: 1.0, "f1-score": 1.0, support: 60 },
        sadness: {
          precision: 0.9836,
          recall: 1.0,
          "f1-score": 0.9917,
    ...
    return {
      success: true,
      stdout:
        `[1/3] Epoch 1: Feature Extraction & CNN Alignment | Loss: 1.4230 | Val Acc: 87.50%\n` +
    ...
    try {
      ...
      if (samples.length === 0) {
        ...
        if (fs.existsSync(tessBase)) {
          const dirs = fs
            .readdirSync(tessBase, { withFileTypes: true })
  ...
  function computeFallbackSER(
    ...
    if (
      lowerHint.includes("happy") ||
      lowerHint.includes("joy") ||
      lowerHint.includes("laugh")
    )
      detected = "happiness";
    else if (
      lowerHint.includes("angry") ||
      lowerHint.includes("anger") ||
      lowerHint.includes("shout") ||
      lowerHint.includes("yell")
    )
      detected = "anger";
    else if (
      lowerHint.includes("fear") ||
      lowerHint.includes("scared") ||
      lowerHint.includes("panic") ||
      lowerHint.includes("fright")
    )
      detected = "fear";
    else if (
      lowerHint.includes("sad") ||
      lowerHint.includes("cry") ||
      lowerHint.includes("tears") ||
      lowerHint.includes("grief")
    )
      detected = "sadness";
    else if (
      lowerHint.includes("disgust") ||
      lowerHint.includes("gross") ||
      lowerHint.includes("nasty")
    )
      detected = "disgust";
    else if (
      lowerHint.includes("surpris") ||
      lowerHint.includes("wow") ||
      lowerHint.includes("shock") ||
      lowerHint.includes("_ps")
    )
      detected = "surprise";
    else {
      const emotions = [
    ...
    async (req, res) => {
      try {
        ...
        if (audioBase64) {
          ...
          try {
            await execAsync(
              `ffmpeg -y -i "${rawTempPath}" -ar 16000 -ac 1 -c:a pcm_s16le "${normWavPath}" 2>/dev/null`,
            );
        ...
        if (servedUrl) {
          ...
          result.audioUrl = audioBase64.startsWith("data:")
            ? audioBase64
...
Return ONLY valid JSON without markdown fencing.`;
            ...
            console.warn(
              "Gemini call failed or rate limited, using simple fallback engine:",
              geminiError.message,
            );
          }
        }
        ...
        const simpleEmotionData: Record<
          string,
          {
            summary: string;
            prosody: string;
        ...
        > = {
          happiness: {
            summary:
              "This person sounds very happy and cheerful! Their voice is bright, warm, and full of positive energy.",
            prosody:
              "The voice has a lively bounce, normal to high pitch, and sounds clear and upbeat.",
            valence: 0.85,
            arousal: 0.75,
            desc: "Happy & Energetic",
            meaning:
              "The person is in a great mood, feels satisfied, and enjoys speaking.",
          ...
          sadness: {
            summary:
              "This person sounds sad or down. Their voice is quiet, slow, and low in energy.",
            prosody:
              "The voice sounds soft and gentle, with longer pauses between words.",
            valence: -0.8,
            arousal: 0.25,
            desc: "Sad & Quiet",
            meaning:
              "The person might be feeling hurt, tired, lonely, or upset.",
          ...
          anger: {
            summary:
              "This person sounds angry or very frustrated. Their voice is loud, sharp, and strong.",
            prosody:
              "The voice has heavy force, fast delivery, and sharp, firm words.",
            valence: -0.75,
            arousal: 0.85,
            desc: "Angry & Loud",
            meaning:
              "The person feels upset, irritated, or wants to push back against a problem.",
          ...
          fear: {
            summary:
              "This person sounds scared or anxious. Their voice has nervous, shaky energy.",
            prosody:
              "The voice is slightly unsteady with quick breath and sudden high tones.",
            valence: -0.7,
            arousal: 0.8,
            desc: "Scared & Nervous",
            meaning:
              "The person is worried about something and may feel unsafe or uncertain.",
          ...
          surprise: {
            summary:
              "This person sounds surprised! Something unexpected just caught their attention.",
            prosody:
              "The voice jumps up in pitch quickly, like when someone gasps or opens their eyes wide.",
            valence: 0.5,
            arousal: 0.85,
            desc: "Surprised & Alert",
            meaning:
              "The person just heard or saw something new, surprising, or unexpected.",
          ...
          disgust: {
            summary:
              "This person sounds displeased or disgusted. They do not like what they are talking about.",
            prosody:
              "The voice is drawn out with a low, slightly harsh tone that shows dislike.",
            valence: -0.65,
            arousal: 0.5,
            desc: "Displeased & Hesitant",
            meaning:
              "The person strongly rejects an idea, smell, taste, or situation.",
          ...
          neutral: {
            summary:
              "This person sounds calm, relaxed, and balanced. They are speaking in a normal everyday voice.",
            prosody:
              "The voice is steady, smooth, and even in volume, without any big emotional swings.",
            valence: 0.05,
            arousal: 0.35,
            desc: "Calm & Balanced",
            meaning:
              "The person is relaxed, focused on sharing information, and not stressed.",
        ...
        const emoData =
          simpleEmotionData[predictedEmotion] || simpleEmotionData.neutral;
  ...
  app.post("/api/run-script", async (req, res) => {
    try {
      const scriptPath = path.join(
        process.cwd(),
        "ml",
        "01_kaggle_setup_and_download.py",
      );
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(
      `Speech Emotion Recognition server running on http://localhost:${PORT}`,
    );
  });
}
